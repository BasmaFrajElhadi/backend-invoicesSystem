								...
								<div class="dropdown-menu">
									<div class="menu-header-content bg-primary text-right">
										<div class="d-flex">
                                            ...
                                            @if (Auth::user()->unreadNotifications->count())
											    <span class="badge badge-pill badge-warning mr-auto my-auto float-left"><a href="{{route('MarkAsRead_all')}}" style="color:black"> تعين فراءة الكل</a></span>
                                            @endif
										</div>
										<p class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12 ">لديك {{Auth::user()->unreadNotifications->count()}} اشعارات غير مقروءة</p>
									</div>
									<div class="main-notification-list Notification-scroll">
                                        @foreach (Auth::user()->unreadNotifications as $notification)
                                            <a class="d-flex p-3 border-bottom" href="{{url('/showNotification/'.$notification->data['invoice_id'])}}">
                                                <div class="notifyimg bg-pink">
                                                    <i class="la la-file-alt text-white"></i>
                                                </div>
                                                <div class="mr-3">
                                                    <h5 class="notification-label mb-1">{{$notification->data['title']}}</h5>
                                                    <div class="notification-subtext">تمت بواسطة  {{$notification->data['by_user']}}</div>
                                                    <div class="notification-subtext"> {{ $notification->created_at->format('H:i:s')}}</div>
                                                </div>
                                                <div class="mr-auto" >
